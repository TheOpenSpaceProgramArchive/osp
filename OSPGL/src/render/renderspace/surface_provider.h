	...
	virtual void get_heights(PlanetTilePath& path, size_t verts, std::vector<float>& out, const Planet& planet)
	{
		...
		for (int iy = -1; iy < (int)verts + 1; iy++)
		{
			for (int ix = -1; ix < (int)verts + 1; ix++)
			{
				...
				height = stb_perlin_ridge_noise3(side.x * 250.0f, side.y * 250.0f, 1.0f, 2.0f, 0.7f, 0.9f, 8) * 0.0005f + 
					stb_perlin_fbm_noise3(side.x * 58.0f, side.y * 58.0f, 2.0f, 2.0f, 0.5f, 8) * 0.00002f +
					stb_perlin_fbm_noise3(side.x * 8.0f, side.y * 8.0f, 3.0f, 2.0f, 0.5f, 8) * 0.0005f +
					stb_perlin_fbm_noise3(side.x * 2.0f, side.y * 2.0f, 3.0f, 2.0f, 0.5f, 8) * 0.0005f -
					stb_perlin_ridge_noise3(side.x * 8.0f, side.y * 8.0f, 1.0f, 2.0f, 0.5f, 0.6f, 8) * 0.15f + 
					stb_perlin_fbm_noise3(side.x * 600.0f, side.y * 600.0f, 5.0f, 3.0f, 0.6f, 8) * 0.000005f;
